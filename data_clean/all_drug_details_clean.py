...
def getCleanDetails():
    input_file = "/Users/anjin/work/git_anjin/drug_recommendation_system/scrapy/drug/drug_details_revised.json"
    list = []
    with open(input_file) as p:
        ...
        for tuple in dict:
            temp = {}
            if len(tuple['drug_name']) == 0 or tuple['drug_name'].lower() in drug_set:
                continue
            drug_set.add(tuple['drug_name'].lower())
            temp["drug_name"] = tuple['drug_name']
